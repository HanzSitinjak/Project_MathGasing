...
{
    public function dataLeaderboardAll()
    {
        ...
        $badgeSubquery = LencanaUser::select('id_user', DB::raw('COUNT(id_bagde) as total_badges'))
            ->groupBy('id_user');

        $leaderboardData = DB::table(DB::raw("({$scorePosttestQuery->toSql()}) as score_posttests"))
            ->mergeBindings($scorePosttestQuery->getQuery())
            ->leftJoinSub($badgeSubquery, 'badge_counts', 'score_posttests.id_user', '=', 'badge_counts.id_user')
            ->leftJoin('user', 'score_posttests.id_user', '=', 'user.id_user')
            ->select('score_posttests.id_user', 'user.name', 'score_posttests.total_score_posttest', 'badge_counts.total_badges')
            ->orderBy('total_score_posttest', 'desc')
            ->get();
