...
class ScorePretestController extends Controller
{
    ...
    {
        $subquery = ScorePretest::selectRaw('MIN(id_ScorePreTest) as min_id, id_user, id_pretest, id_unit')
            ->groupBy('id_user', 'id_pretest', 'id_unit');
    
        $scorePretest = ScorePretest::joinSub($subquery, 'min_scores', function ($join) {
                $join->on('score_pretest.id_ScorePreTest', '=', 'min_scores.min_id');
            })
            ->select('score_pretest.*')
            ->get();
    
        return response()->json(['data' => $scorePretest]);
    }

    public function show($id){
        $scorePretest = ScorePretest::findOrFail($id);
        return response() -> json(['data' => $scorePretest]);
    }
    ...
    public function addScore(Request $request){
        ...
        $scorePretest = ScorePretest::create([
        ...
        return response()->json(['data' => $scorePretest], 201);
    }
