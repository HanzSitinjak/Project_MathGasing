...
class BadgeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $existingBadge = Badge::where('id_posttest', $request->id_posttest)->first();
        if ($existingBadge) {
            return response()->json(['error' => 'data ini sudah ada sebelumnya'], 400);
        }
    ...
    public function update(Request $request, $id)
{
    ...
    // Periksa apakah id_posttest yang baru sudah ada di database dan bukan milik data yang sedang diupdate
    $existingBadge = Badge::where('id_posttest', $request->id_posttest)->where('id_bagde', '!=', $id)->first();
    if ($existingBadge) {
        return response()->json(['error' => 'data ini sudah ada sebelumnya'], 400);
    }
    ...
    public function showByMateri($id_materi)
    {
        $badges = Badge::where('id_materi', $id_materi)->get();

        if ($badges->isEmpty()) {
            return response()->json(['message' => 'No badges found for this materi ID'], 404);
        }
